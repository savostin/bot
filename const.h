...
enum StrategyType { ST_BJ_LF, ST_BJT_LF };
...
enum ChannelStatus { UNDEFINED = 0, STOPPED, RUNNING, PAUSED };
...
enum StatementType { PL, COMMISSION, DEPOSIT, WITHDRAWAL };
  ...
  json toJson() {
    json j = {
        ...
        {"timestamp",
         fmt::format(
             "{:%FT%T%z}",
             fmt::localtime(chrono::system_clock::to_time_t(timestamp)))},
        {"secret",
         {
             {"type", type},
...
enum EventType { E_NONE, E_EXIT, E_PAUSE, E_RESUME };
