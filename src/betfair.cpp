...
map<string, float> BetFairAccount::minBetAmount{
    {"EUR", 2}, {"DKK", 30}, {"USD", 4},  {"GBP", 2}, {"AUD", 6},
    {"CAD", 6}, {"HKD", 25}, {"NOK", 30}, {"SGD", 6}, {"SEK", 30}};
...
xml_document BetFair::getSnapshot(const int id) {
  ...
  return Request(fmt::format("/rest/v1/channels/{}/snapshot", id),
                 HTTP::no_xml);
}

BetFairAccount::BetFairAccount()
    : BetFair(), username(""), password(""), available(0), running(true), th() {
...
bool BetFairAccount::getFunds() {
  xml_document doc =
      Request(fmt::format("/rest/v1/account/snapshot?username={}", username),
              HTTP::no_xml);
  if (lastStatus == 200) {
...
bool BetFairAccount::placeBet(struct Bet &bet) {
  ...
  logger->warn(_.AccountPlacingBet, sideNameInt, bet.selection.name, bet.amount,
               currency, bet.price);
  xml_document send;
  send.load_string(
      fmt::format(
          "<postBetOrder xmlns=\"urn:betfair:games:api:v1\" marketId=\"{:d}\" "
          "round=\"{:d}\" "
          "currency=\"{}\"><betPlace><bidType>{}</bidType><price>{:.2f}</"
          "price><size>{:.2f}</size><selectionId>{:d}</selectionId></"
          "betPlace></postBetOrder>",
          bet.market.id, bet.round, currency, sideName, bet.price,
          minBet(bet.amount), bet.selection.id)
          .c_str());
  xml_document doc =
      Request(fmt::format("/rest/v1/bet/order?username={}", username), send);
  xml_node betResult =
      doc.child("responseBetOrder").child("betPlacementResult");
  if (betResult) {
    bet.status = string(betResult.child_value("resultCode")) == string("OK")
                     ? BetMatched
                     : BetError;
    bet.id = atol(betResult.child_value("betId"));
    logger->warn(_.AccountBetResult, betResult.child_value("resultCode"),
                 betResult.child_value("betId"));
    getFunds();
    logger->warn(_.AccountFunds, available, currency);
    return true;
  } else {
...
vector<Bet> BetFairAccount::getBetsHistory(const string &status) {
  ...
  xml_document doc =
      Request(fmt::format("/rest/v1/bet/history?username={}&betStatus={}",
                          username, status),
              HTTP::no_xml);
  if (doc) {
    xml_node root = doc.child("betHistory");
    for (xml_node item = root.child("betHistoryItem"); item;
         item = item.next_sibling("betHistoryItem")) {
      Bet b;
      b.id = stol(item.child_value("betId"));
      b.side =
          string(item.child_value("bidType")) == string("BACK") ? BACK : LAY;
      ...
      logger->debug("Got bet {:d}: '{}' {:.2f} @ {:.2f} in {:d} round placed "
                    "{:%Y-%m-%d %H:%M:%S}. Result: {} with p/l {:.2f}",
                    b.id, b.selection.name, b.amount, b.price, b.round,
                    fmt::localtime(b.placed),
                    (b.status == BetWon ? "WON" : "LOST"), b.pl);
...
vector<Bet> BetFairAccount::getBets(const unsigned long channel,
                                    const string &status) {
  vector<Bet> bets;
  logger->debug("Getting current {} bets for channel {}", status, channel);
  xml_document doc = Request(
      fmt::format(
          "/rest/v1/bet/snapshot?username={}&betStatus={}&channelId={:d}",
          username, status, channel),
      HTTP::no_xml);
  if (doc) {
    xml_node root = doc.child("betSnapshot");
    for (xml_node item = root.child("betSnapshotItem"); item;
         item = item.next_sibling("betSnapshotItem")) {
      Bet b;
      b.id = stol(item.child_value("betId"));
      b.side =
          string(item.child_value("bidType")) == string("BACK") ? BACK : LAY;
      ...
      b.pl = 0;
      b.round = stoi(item.child_value("roundNumber"));
      b.status = string(item.child_value("priceMatched")).empty() ? BetUnmatched
                                                                  : BetMatched;
      b.placed = str2time(item.child_value("placedDate"));
...
vector<Statement> BetFairAccount::getStatement(const int count,
                                               const int from) {
  vector<Statement> statement;
  string typeName = "";
  logger->debug("Getting account statement {} / {}", count, from);
  xml_document doc = Request(
      fmt::format("/rest/v1/account/"
                  "statement?username={}&recordCount={:d}&startRecord={:d}",
                  username, count, from),
      HTTP::no_xml);
  if (doc) {
    xml_node root = doc.child("accountStatement");
    for (xml_node item = root.child("item"); item;
      ...
      xml_node asdb = item.child("accountStatementDescription")
                          .child("accountStatementBetDescription");
      if (asdb) {
        s.type = PL;
        typeName = "P/L";
        s.description = fmt::format(
            "{} - {} {:.2f} {} @ {:.2f} '{}': {}",
            asdb.child("selectionReference")
                .child("marketReference")
                .child_value("channelName"),
            asdb.child_value("bidType"), stof(asdb.child_value("size")),
            currency, stof(asdb.child_value("size")),
            asdb.child("selectionReference").child_value("selectionName"),
            asdb.child_value("winLose"));
      }
      xml_node asdd = item.child("accountStatementDescription")
                          .child("accountStatementTransferDescription");
      if (asdd) {
        s.type = string(asdd.child_value("paymentType")) == string("DEPOSIT")
                     ? DEPOSIT
                     : WITHDRAWAL;
        typeName = asdd.child_value("paymentType");
        s.description = "";
      }
      xml_node asdc = item.child("accountStatementDescription")
                          .child("accountStatementCommissionDescription");
      if (asdc) {
        s.type = COMMISSION;
        typeName = "COMMISSION";
        s.description = fmt::format(
            "{}% - {}", asdc.child_value("commissionRate"),
            asdc.child("marketReference").child_value("channelName"));
      }
      logger->debug(
          "Got record {:d}: {:%Y-%m-%d %H:%M:%S} {} {:+.2f} {:.2f} {}", s.id,
          fmt::localtime(s.timestamp), typeName, s.amount, s.balance,
          s.description);
