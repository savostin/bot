...
void signal_handler(int) { Events::loop()->notify(E_EXIT); }
...
int main(int argc, char **argv) {
  ...
  if (args["help"]) {
    cerr << fmt::format((string)_.Usage, argv[0]) << endl << argparser;
    return EXIT_SUCCESS;
  }

  if (args["version"]) {
    cout << VERSION_NAME << " v." << VERSION_MAJOR << "." << VERSION_MINOR
         << "." << VERSION_PATCH << endl;
    return EXIT_SUCCESS;
  }
  ...
  if (o.get("proxy_port", 0) > 0 && !o.get("proxy_server", "").empty()) {
    HTTP::setProxy(o.get("proxy_server", ""), o.get("proxy_port", 0),
                   o.get("proxy_username", ""), o.get("proxy_password", ""));
  }
  ...
  logger->info(_.Welcome, VERSION_NAME, VERSION_MAJOR, VERSION_MINOR,
               VERSION_PATCH);
