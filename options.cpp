...
bool Options::load()
{
    DB::o() << "select v from data where n = 'options';" >>
        [&](string v) {
            try
            {
                data = json::parse(v);
                cout << "Load options:  " << v << endl
                     << data << endl;
            }
            catch (...)
            {
...
void Options::save()
{
    ...
    cout << "!" << endl;
    DB::o() << "insert into data (n, v) values ('options', ?);"
            << data.dump();
    cout << "!" << endl;
    } catch (...) {
        cerr << "WTF" << endl;
...
void Options::set(std::string name, int val)
{
    data[name] = val;
}

void Options::set(std::string name, std::string val)
{
    data[name] = val;
}
